...
def register_user(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			login(request, user)
			messages.sucess(request, "You Have Successfully Registered. Welcome!")
			return redirect('home')
	else: 
		form = SignUpForm()
...
def album_record(request, pk):
	if request.user.is_authenticated:
		album_record = Record.objects.get(id=pk)
		return render(request, 'record.html', {'album_record':album_record})
	else: 
